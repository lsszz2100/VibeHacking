...
> 🎮 **ブラウザのターミナルで遊ぶ侵入CTFワーゲーム** — 偽シェル（`connect`・`cat`・`submit`）でターゲット `vibe.corp` のセキュリティ層5つ（外周→Web→内部→金庫→コア）を1層ずつ突破し、1015 問をクリア。起動シーケンス・`ACCESS GRANTED` 演出・マトリックスレイン・サウンド・ヒント・日英対応。本リポジトリの75セクション基盤。([紹介](wargame/README.md) · GitHub Pages有効化後にアクセス可能)
