...
**로그 우회**는 공격자가 자신의 활동 흔적을 숨기거나 삭제하는 기술입니다.

```
왜 로그가 중요한가:
  방어자: "누가, 언제, 무엇을 했는가?" 추적
  공격자: 이 추적을 차단 → 탐지/대응 지연
  
로그 우회 없이:
  침해 발생 → 로그 분석 → 공격자 행동 재구성 → 즉각 대응
  
로그 우회 성공 시:
  침해 발생 → 로그 없음 → 무슨 일이 일어났는지 모름
  → 피해 범위 파악 어려움
  → 공격자가 더 오래 잠복
```
...
공격자: Windows 이벤트 로그 삭제
방어자: SIEM에 실시간 전송 → 삭제해도 이미 보존됨

공격자: PowerShell 로깅 우회
방어자: AMSI, Script Block Logging → 우회 어려워짐

공격자: 타임스탬프 조작
방어자: 여러 소스 타임라인 교차 검증

교훈: 완벽한 로그 우회는 불가능
     방어팀이 잘 구성되어 있으면 반드시 흔적 남음
```

### 레드팀에서 로그 우회를 배우는 이유

```
방어팀이 설정한 탐지 규칙의 효과 평가:
  "로그 설정이 제대로 됐는가?"
  "공격자 기법을 우리가 탐지할 수 있는가?"
  
레드팀 → 로그 우회 시도 → 블루팀 탐지 테스트
→ 탐지 실패 시 → 탐지 규칙 개선
→ 탐지 성공 시 → 방어 능력 검증
```
...
**최신 기법·통제 (2025–2026):**
- ETW 패치·EDR 센서 언로드에 대응해 불변(WORM) 원격 수집이 필수 — 로컬 삭제(EID 1102) 후에도 SIEM 원본이 남는지가 실제 방어선
- Sysmon 서비스 상태/설정 변경 감시로 텔레메트리 무력화를 포착: `Get-WinEvent -LogName 'Microsoft-Windows-Sysmon/Operational' | ? {$_.Id -in 4,16}` (4=서비스 상태 변경, 16=설정 변경)
