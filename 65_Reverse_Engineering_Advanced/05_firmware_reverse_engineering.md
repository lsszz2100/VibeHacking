...
**최신 기법·통제 (2025–2026):**
- Secure Boot·서명 검증·암호화 펌웨어·eFuse 잠금이 기본화 — 추출/마운트가 아니라 서명 우회·복호화가 실제로 되는지 확인. 검증: `binwalk -E`(엔트로피=암호화 여부)·`unblob` 후 파일시스템 마운트·부트체인 서명 확인
- UART/JTAG/SPI 플래시 덤프 등 하드웨어 공격면과 결합 — 소유·허가 기기 한정, 추출본이 실제 실행 이미지와 일치하는지 해시로 대조([[27_IoT_Hacking]])
