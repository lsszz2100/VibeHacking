...
**최신 기법·통제 (2025–2026):**
- CTI→에뮬레이션 플랜으로 특정 위협행위자 TTP를 재현(Caldera·VECTR) — "도구를 돌렸다"가 아니라 실제 행위자와 동일한 신호가 나는지 텔레메트리 패리티를 확인
- EDR 회피·LOTL(정상도구 악용) 기법 에뮬레이션으로 행위탐지의 견고성 시험 — 난독화·변형 버전에서도 탐지가 살아남는지 검증([[75_Red_Team_Reporting]])
