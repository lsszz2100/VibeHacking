...
**OSINT 방법론**은 체계적으로 공개 정보를 수집하고 분석하는 절차입니다. 아무리 좋은 도구가 있어도 방법론 없이 쓰면 정보가 산발적으로 흩어집니다.

**무작위 수집 vs 방법론적 수집:**
```
무작위:
  구글 검색 → 링크드인 검색 → 소호 가서 점심
  → 얻은 것: 단편 정보들, 연결 고리 없음
  
방법론적:
  1. 목표 정의: "이 회사의 외부 공격 표면은?"
  2. 씨앗 정보: company.com 도메인
  3. 확장: 도메인 → 서브도메인 → IP → 서비스 → 직원 → 이메일
  4. 분석: 연결 관계 파악, 취약점 식별
  5. 문서화: 구조화된 프로파일
  → 얻은 것: 완전한 공격 지도
```
...
**Google Dorking**은 구글 검색 연산자를 사용해 일반 검색으로는 찾기 어려운 정보를 찾는 기법입니다.

```
일반 검색: samsung 채용
Google Dork: site:samsung.com filetype:pdf "채용" "2026"
→ 삼성 공식 사이트의 2026년 PDF 채용 공고만 검색

위험한 Dork 예시:
site:target.com filetype:sql          → 노출된 SQL 파일
site:target.com inurl:admin            → 관리자 페이지
site:target.com "Index of" /backup    → 백업 디렉터리
site:target.com ext:env               → .env 설정 파일

이런 파일들이 공개 인터넷에 노출되는 이유:
  - 개발자 실수 (gitignore 누락)
  - 서버 설정 오류
  - 자동화 스크립트 오류
```
...
1. 계획:
   무엇을 알고 싶은가? (PIR: Priority Intelligence Requirements)
   어떤 수집 방법을 사용할 것인가?
   
2. 수집:
   Google Dork, Shodan, LinkedIn, GitHub, DNS 등
