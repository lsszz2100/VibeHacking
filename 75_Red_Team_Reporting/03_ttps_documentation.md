...
**최신 기법·통제 (2025–2026):**
- ATT&CK 매핑·기계판독(Navigator 레이어·STIX)으로 TTP를 구조화 — 방어 갭분석에 직결. 검증: 각 TTP가 관측 텔레메트리·탐지결과와 연결되는가
- 재현 가능한 절차·아티팩트 기록이 퍼플팀 재실행의 전제 — 문서가 데이터로 검증되는지 확인([[68_Purple_Team]])
