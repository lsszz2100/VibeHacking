...
라이트업(Writeup)은 CTF 문제를 어떻게 풀었는지 단계별로 기록한 문서다. 대회가 끝난 후 커뮤니티에 공유하는 것이 CTF 문화의 핵심이다. 좋은 라이트업은 "왜 그 접근법을 선택했는가"까지 설명해 독자가 같은 사고 과정을 따라갈 수 있게 한다. 라이트업 작성 자체가 최고의 학습 방법이기도 하다.
...
라이트업이 중요한 이유:
  ...
  본인 관점:
    - 풀이 과정 체계화 → 실력 향상
    - 포트폴리오 구성 (취업/인턴십에 직접 활용)
    - 나중에 유사 문제 빠르게 풀 수 있는 메모

  커뮤니티 관점:
    - 다른 참가자의 학습 자원
    - 대회 주최자의 다음 문제 개선 피드백
    - 글로벌 보안 지식 공유

  취업 관점:
    - CTFtime.org 프로필 + GitHub 라이트업
      = 실무 능력 입증 포트폴리오
    - 채용 담당자가 직접 확인하는 자료
```
...
### 필요한 도구 및 환경
- **Obsidian / Notion**: 라이트업 초안 작성 및 정리
- **GitHub**: 라이트업 공개 저장소 운영
- **Typora / VS Code**: 마크다운 편집기
- **asciinema**: 터미널 세션 녹화 (과정 시각화)
- **CyberChef**: 중간 과정 스크린샷 대신 링크로 공유 가능

### 기초 실습 예제
```python
#!/usr/bin/env python3
"""
CTF 라이트업 마크다운 템플릿 자동 생성기
문제 정보를 입력하면 구조화된 라이트업 초안을 생성한다.
"""
from datetime import datetime


def generate_writeup_template(
    ctf_name: str,
    challenge_name: str,
    category: str,
    points: int,
    ...
    """CTF 라이트업 마크다운 초안을 생성한다."""
    today = datetime.now().strftime("%Y-%m-%d")

    template = f"""# [{ctf_name}] {challenge_name}
...
- [이 문제에서 새로 알게 된 기술/개념]
- [다음에 유사 문제 만났을 때 기억할 것]
...
if __name__ == "__main__":
    writeup = generate_writeup_template(
        ctf_name="picoCTF 2024",
        challenge_name="heap_overflow",
        category="Pwn",
        points=300,
    ...
    print(writeup)
    # 파일로 저장
    with open("writeup_template.md", "w", encoding="utf-8") as f:
        f.write(writeup)
    print("\n[writeup_template.md 생성 완료]")
```
