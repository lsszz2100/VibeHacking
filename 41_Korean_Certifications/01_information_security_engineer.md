...
**최신 기준 (2025–2026):**
- 최신 시험은 제로트러스트·클라우드·공급망 보안 비중이 커짐 — 접근통제 도메인은 최소권한을 넘어 **지속 검증(제로트러스트)** 관점으로 세그멘테이션·MFA 적용까지 확인
- 암호 도메인은 TLS 1.3 기본화·양자내성암호(PQC) 전환 논의를 반영 — 약한 스위트뿐 아니라 프로토콜 버전·인증서 수명까지 점검
