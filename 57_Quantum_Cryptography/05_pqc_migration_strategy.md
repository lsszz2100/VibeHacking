...
|-----------|-----------|---------|------------|---------|
| **군사위성 TT&C 명령** | **15~20년 (위성 수명)** | **최高** | **최高** | **즉시** |
...
> **군사위성 사례**: ANASIS-II는 운용 수명 15~20년으로, 발사 시점 암호화 트래픽이
> 2040년대 CRQC 출현 후 복호화될 수 있는 **HNDL 위협**에 직접 노출된다.
> 이 때문에 X25519 + ML-KEM-768 하이브리드 KEM으로 세션 키를 수립하는 것이
> 위성 PQC 전환에서 최우선 과제다.
...
### 5.4 위성 시스템 PQC 전환 특수 위험 (연구 사례)

> **출처**: ANASIS-II 위성 PQC 프레임워크 구현 연구 (2026-05)

지상 시스템과 다른 위성 특화 위험 요소:

| 위험 요소 | 위성 특수성 | 대응 전략 |
|---------|-----------|---------|
| **OTA 업데이트 불가** | 발사 후 펌웨어 교체 어려움 | SLH-DSA Stateless 서명으로 검증; 지상국에서 서명 후 업링크 |
| **메모리 제약** | LEON4FT 기준 7.2KB RAM | ML-KEM 공개키 1,184B — 메모리 허용; SLH-DSA 서명(16,224B)은 OBC에 저장 않음 |
| **방사선 SEU** | 우주 방사선에 의한 비트 플립 | TMR 키 스토어: 세 복사본 다수결로 단일 비트 플립 자동 복구 |
| **링크 대역폭** | S-band 2Mbps 업링크 | ML-DSA-65 서명 3,309B → TC 프레임 3~4개 분할; CCSDS SDLS L2에서 처리 |
| **단방향 전파 지연** | GEO 약 270ms (지상-위성 왕복 540ms) | Nonce 캐시 + 30초 타임스탬프 윈도우로 재전송 방지 |
| **PKI 키 교체** | 위성과의 물리적 키 전달 불가 | 3단계 PKI (Root→Mission→OBC), 30일 교체 주기, OTA 전달 |

**ProVerif 형식 검증으로 확인한 설계 위험**:
- V2b (ML-DSA-87 포함): 8/8 PROVED — 유일하게 형식 검증 완료된 안전한 설계
- V2c (KEM만, 서명 제거): P3/P4/P5 FALSE — UKS(Unknown Key Share) 공격에 취약
  → 대역폭 64.5% 절감 유혹에도 군사위성에서는 V2c 채택 금지
