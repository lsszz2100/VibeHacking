...
**최신 기법·통제 (2025–2026):**
- 하드웨어 브레이크포인트 탐지·타이밍(rdtsc/rdtscp)·VM/하이퍼바이저 아티팩트 검사 등 다층 안티디버깅이 결합됨 — 단일 패치가 아니라 전 경로가 죽었는지 재현으로 확인. 검증: `qiling`/`unicorn` 에뮬레이션으로 탐지 콜 우회를 관찰
- eBPF·하이퍼바이저 기반 스텔스 디버깅으로 유저모드 탐지를 회피 — 안티디버깅이 실제로 유효한지 대상 기준 재현이 관건([[06_Malware_Analysis]])
