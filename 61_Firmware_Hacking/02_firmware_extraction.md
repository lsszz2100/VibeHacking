...
**펌웨어 추출**은 장치의 플래시 메모리에서 펌웨어 데이터를 가져오는 과정입니다. 분석하기 전에 먼저 펌웨어를 확보해야 합니다.

```
왜 추출이 필요한가:
  장치 그 자체 = 해석하기 어려운 상태
  펌웨어 파일 = 파일시스템 구조로 분석 가능
...
```
쉬움 (추가 하드웨어 불필요):
  1. 벤더 공식 업데이트 파일 다운로드
     → 제조사 웹사이트에서 firmware.bin 다운로드
     → binwalk로 바로 분석 가능
...
중간 (네트워크 필요):
  3. OTA(Over-The-Air) 업데이트 캡처
     → 장치가 업데이트 받을 때 트래픽 가로채기
     → Wireshark로 HTTPS 복호화 필요
...
```
binwalk는 펌웨어 분석의 필수 도구입니다.

주요 기능:
  - 파일 시그니처 탐지 (파일 유형 자동 인식)
  - 내장된 파일시스템, 이미지, 스크립트 추출
  - 엔트로피 분석 (암호화/압축 영역 탐지)
  
사용 예시:
  binwalk firmware.bin          → 구조 분석
  binwalk -e firmware.bin       → 파일 자동 추출
  binwalk --entropy firmware.bin → 암호화 영역 시각화
```
