...
리눅스는 터미널(검은 화면)에서 텍스트 명령어를 입력해 시스템을 제어하는 운영체제입니다. 보안 전문가들이 리눅스를 사용하는 이유는 서버 대부분이 리눅스로 운영되고, 해킹·방어 도구의 대부분이 리눅스 전용으로 개발되기 때문입니다.

**왜 배우는가:**
```
[일반 사용자]  마우스로 클릭 → 속도 느림, 자동화 불가
[보안 전문가]  명령어 입력   → 빠른 작업, 스크립트 자동화

실제 활용:
  파일 시스템 탐색  → 해킹 후 타겟 서버 내부 파악
  권한 확인        → 취약한 SUID 파일 발견
  네트워크 확인    → 열린 포트, 연결 상태 점검
  프로세스 관리    → 악성 프로세스 탐지 및 종료
```
...
├── /etc           (시스템 설정 파일)
├── /home          (일반 사용자 홈 디렉토리)
├── /var/log       (시스템 로그 — 해킹 흔적 확인)
├── /tmp           (임시 파일 — 악성코드 종종 여기에 저장)
├── /usr/bin       (일반 실행 파일)
└── /root          (root 사용자 홈)

권한 표기법:
  -rwxr-xr--   파일유형 + 소유자권한 + 그룹권한 + 기타권한
   rwx = 읽기(r=4) + 쓰기(w=2) + 실행(x=1)
   예: chmod 755 → 소유자 모든 권한 / 나머지 읽기+실행
```

### 필요한 도구 및 환경
- **터미널**: 리눅스/macOS 기본 제공, Windows는 WSL2 또는 가상머신 사용
- **Kali Linux**: 보안 전용 배포판 — 대부분의 해킹 도구 사전 설치
- **가상머신**: VirtualBox 또는 VMware로 안전한 실습 환경 구성
...
# 파일 탐색 기초
ls -al          # 숨김 파일 포함 상세 목록 (권한, 소유자, 크기)
ls -alh         # 파일 크기를 KB/MB 단위로 표시

# 보안 관점의 파일 탐색
find / -perm -4000 -type f 2>/dev/null  # SUID 파일 검색 (권한 상승 단서)
find /tmp -type f 2>/dev/null           # /tmp 파일 확인 (악성 파일 탐지)
```
...
**최신 기법·통제 (2025–2026):**
- 최신 배포판은 Wayland·systemd 샌드박싱·불변(immutable) 루트파일시스템으로 기본 강화 — 기본 명령 동작이 격리정책의 영향을 받음. 검증: 권한상승/파일접근이 auditd·journald에 실제 기록되는지 재현(소유/랩 한정)([[26_Linux_Hardening]])
- 셸 히스토리·환경변수·`/proc/*/cmdline`으로 시크릿 노출이 흔함 — 자격증명은 명령라인 대신 헬퍼/권한 600 파일 사용, 노출 여부를 스캔으로 확인
