...
**최신 기법·통제 (2025–2026):**
- 가드레일(입력/출력 필터·LLM심판·정책엔진)과 관측성(프롬프트/툴호출 로깅)의 결합이 방어 표준 — 각 계층이 우회되는지 레드팀으로 재검증
- 런타임 격리(샌드박스 툴실행·권한경계·콘텐츠 격리)가 핵심 — 통제가 배포됐다가 아니라 실제 공격경로를 끊는지 재현으로 확인([[48_Threat_Modeling]])
