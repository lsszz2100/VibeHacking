...
**라우팅**은 패킷이 출발지에서 목적지까지 가는 "경로"를 결정하는 과정입니다. 택배 배송 경로를 결정하는 것과 비슷합니다.

```
비유: 택배 회사 네트워크
  서울 물류센터 → (최적 경로) → 부산 물류센터 → 수신자

라우팅:
  PC → 라우터 → (최적 경로 계산) → 목적지 서버
  
경로 결정 방식:
  - 정적 라우팅: 관리자가 직접 경로 입력 (변경 없음)
  - 동적 라우팅: 라우터끼리 정보 교환해서 자동 계산 ← 공격 대상!
```

### 동적 라우팅 프로토콜이란?

라우터들이 서로 "어떤 네트워크를 알고 있는지" 자동으로 정보를 교환합니다.

```
라우터 A: "나는 10.0.1.0/24 네트워크를 알아"
라우터 B: "그 정보 나한테도 줘" → 라우팅 테이블 업데이트
라우터 C: "나는 192.168.1.0/24를 통해 인터넷 연결돼"

→ 모든 라우터가 전체 네트워크 경로 파악
```

**문제점:** "이웃 라우터를 신뢰한다"는 가정 → 공격자가 가짜 경로 정보를 뿌리면 모든 라우터가 속을 수 있습니다.

### 라우팅 프로토콜 종류

| 약어 | 이름 | 사용 범위 |
|------|------|-----------|
| OSPF | Open Shortest Path First | 내부 네트워크 (대기업/ISP) |
| EIGRP | Enhanced IGRP | Cisco 장비 내부 네트워크 |
| BGP | Border Gateway Protocol | 인터넷 (ISP 간 경로 교환) |
| HSRP | Hot Standby Router Protocol | 게이트웨이 이중화 (Cisco) |
| VRRP | Virtual Router Redundancy Protocol | 게이트웨이 이중화 (표준) |

### 공격으로 가능한 것들

```
1. 트래픽 가로채기 (MitM)
   정상 경로: PC → 라우터 A → 인터넷 서버
   공격 후: PC → 라우터 A → [공격자 라우터] → 인터넷 서버
   결과: 모든 트래픽이 공격자를 경유

2. 서비스 거부 (DoS)
   Null 라우트 주입: 특정 목적지로 가는 경로를 '블랙홀'로 변경
   결과: 그 서비스에 아무도 접근 불가

3. 트래픽 블랙홀링
   2008년 파키스탄 텔레콤이 YouTube IP 대역을 자국으로 라우팅
   결과: 전 세계 YouTube 트래픽이 파키스탄으로 몰려 서비스 중단
```
...
**실제 사례:**
- **2008년 파키스탄 텔레콤 유튜브 블랙홀링**: BGP 오설정으로 전 세계 YouTube 접속 2시간 차단
- **2018년 Amazon Route 53 BGP 하이재킹**: 암호화폐 지갑 사이트 트래픽 탈취
- **2024년 중국계 APT BGP 조작 의혹**: 미국 통신사 경유 트래픽 모니터링
...
```
1. 이웃 발견 (Hello 패킷)
   - 라우터가 주기적으로 Hello 패킷 브로드캐스트
   - 같은 Hello 받은 라우터가 "이웃(Neighbor)"이 됨

2. 토폴로지 정보 교환 (LSA)
   - 각 라우터가 자신이 연결된 링크 정보(LSA)를 전파
   - 모든 라우터가 동일한 LSDB(링크 상태 데이터베이스) 보유
...
**공격 포인트:**  
OSPF는 LSA를 보낸 라우터가 정말 그 경로를 가지고 있는지 검증하지 않습니다 (인증 없을 때). 공격자가 가짜 LSA를 보내면 이웃 라우터들이 그대로 믿습니다.
