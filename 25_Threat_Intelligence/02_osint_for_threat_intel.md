...
**OSINT(Open Source INTelligence, 오픈소스 인텔리전스)**는 공개적으로 접근 가능한 정보를 수집하고 분석하는 활동입니다.
...
### OSINT를 위협 인텔리전스에 활용하는 방법

```
시나리오: 우리 회사에 스피어피싱 공격이 발생했음

방어 목적 OSINT:
  1. 공격에 사용된 도메인 분석
     → Whois 조회: 언제 등록됐나?
     → passive DNS: 이전에 어떤 IP를 가리켰나?
     → VirusTotal: 이 도메인으로 다른 공격이 있었나?
  
  2. 공격자 인프라 파악
     → Shodan: 이 IP에서 무슨 서비스가 실행 중?
     → 인증서 정보: 같은 인증서를 쓰는 다른 도메인은?
...
```
패시브 정찰 (대상에게 흔적 남기지 않음):
  Shodan/Censys    — 인터넷 전체 스캔 데이터
  Whois/RDAP       — 도메인/IP 등록 정보
  Passive DNS      — DNS 조회 이력
  VirusTotal       — 악성코드/URL/IP 분석
  Maltego          — 관계 시각화
  SpiderFoot       — 자동화된 OSINT 수집

액티브 정찰 (대상에게 직접 요청 → 흔적 남음):
  nmap             — 포트 스캔
  theHarvester     — 이메일/서브도메인 수집
  Recon-ng         — 정찰 프레임워크
  
다크웹 인텔리전스:
  Tor Browser      — .onion 사이트 접근
  전문 CTI 회사의 다크웹 모니터링 서비스
```
...
"Shodan에서 검색하는 것"은 합법
"Shodan으로 찾은 취약한 시스템에 접속하는 것"은 불법
```
...
**Shodan이란?** "사물인터넷의 구글"이라 불리는 서비스로, 인터넷에 연결된 모든 장치를 지속적으로 스캔해서 데이터베이스를 구축합니다.

```
일반 구글: 웹 페이지 색인
Shodan:   IP 주소, 포트, 서비스 배너 색인

검색 예시:
  "default password" country:KR  → 기본 비밀번호 사용 한국 장치
  port:23 country:KR             → 텔넷 열린 한국 장치
  cisco ios                      → Cisco 라우터
  "Powered by" Apache 2.2        → 구버전 Apache
```
