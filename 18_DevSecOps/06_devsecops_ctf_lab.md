...
> - **인터랙티브 실습 랩 12**: [`labs/12_cicd_supply_chain_lab/`](../labs/12_cicd_supply_chain_lab/) — Poisoned Pipeline Execution(PPE), 의존성 혼동(Dependency Confusion), CI/CD 러너 시크릿 탈취, SLSA 빌드 산출물 무단 변조 4단계 웹 콘솔 및 API 실습 (`python3 vhack.py lab start 12`, 웹 콘솔: `http://localhost:8012`)
> - **워게임 트랙**: 워게임 터미널(`wargame/`)의 `supplychain` 트랙 (35개 문제)에서 타이포스쿼팅 탐지, SBOM(CycloneDX) 분석, Cosign 서명 검증 문제를 직접 실습할 수 있습니다.
