...
> 🎮 **浏览器终端渗透 CTF 战争游戏** — 用伪 Shell（`connect`·`cat`·`submit`）逐层攻破目标 `vibe.corp` 的 5 道安全层（外围→Web→内网→金库→核心），通关 1050 道挑战。含启动序列、`ACCESS GRANTED` 演出、矩阵雨、音效、提示与中英文。取材自本仓库的 75 个章节。([介绍](wargame/README.md) · 启用 GitHub Pages 后可访问)
