...
**최신 기법·통제 (2025–2026):**
- 자동화 탈옥(GCG suffix·AutoDAN·다중턴 크레센도)이 성숙 — 정적 거부목록으로는 불충분, 의미기반 분류·다중턴 세션 모니터링 필요. 검증: 최신 탈옥 스위트로 거부율을 정량 측정
- 적대적 예제·백도어(데이터 포이즈닝)가 파인튜닝/RAG 파이프라인에 침투 — 데이터 출처 무결성·SBOM이 실제 강제되는지 확인([[35_Supply_Chain_Attacks]])
