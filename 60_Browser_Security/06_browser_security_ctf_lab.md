...
> 💡 **워게임 트랙 27 `wasm` (WebAssembly & 샌드박스 보안) 연계**:
> 워게임 터미널(`wargame/`)의 27번째 신규 트랙 `wasm` (35개 문제)에서 WebAssembly 바이너리 헤더(0x0061736d), LEB128 정수 디코딩, 선형 메모리 버퍼 오버플로우 오프셋 계산, 간접 호출(`call_indirect`) 테이블 변조, 브라우저 DOM XSS 싱크 주입, Wasm 샌드박스 탈출 기법을 직접 실습할 수 있습니다. (`python3 -m http.server 8000` 후 브라우저 접속)
