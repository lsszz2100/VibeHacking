...
**최신 기법·통제 (2025–2026):**
- angr/Triton·SMT 솔버 발전으로 경로 탐색 자동화가 강해졌으나 경로 폭발·환경 모델링 한계는 여전 — 솔버 해가 실제 바이너리에서 재현되는지 확인이 필수: 생성 입력을 실기 실행해 목표 상태 도달 확인
- 하이브리드 퍼징(퍼저+컨콜릭, 예 SymCC·Fuzzware)이 펌웨어/커널로 확장 — 커버리지 지표뿐 아니라 재현 가능한 크래시로 검증([[30_Vulnerability_Research]])
