...
**최신 기법·통제 (2025–2026):**
- TLS1.3/QUIC로 페이로드 가시성 하락 — JA3/JA4·SNI·인증서·비콘주기 기반 분석으로 전환. 검증: 캡처가 재현 가능한 필터·기대신호와 함께 기록되는가([[40_Threat_Hunting]])
- 대용량 캡처는 Zeek/Arkime 등 메타데이터 파이프라인으로 처리 — 탐지룰이 실제 트래픽에서 발화하는지 검증
