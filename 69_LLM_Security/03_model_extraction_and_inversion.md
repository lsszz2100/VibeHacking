...
**최신 기법·통제 (2025–2026):**
- 쿼리 기반 모델 추출·멤버십추론·모델역전으로 학습데이터/파라미터 유출 — 레이트리밋·쿼리예산·출력혼동·워터마킹으로 방어. 검증: 추출쿼리 패턴이 탐지·차단되는지 측정
- 차등프라이버시(DP-SGD)·연합학습 환경에서도 재구성 공격이 가능 — 프라이버시 예산(ε)이 실제 경험적 누출을 막는지 감사([[31_AI_ML_Security]])
