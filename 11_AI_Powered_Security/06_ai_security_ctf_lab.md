...
> 💡 **바이브해킹 실습 랩 및 워게임 연계**:
> - **인터랙티브 실습 랩 08**: [`labs/08_llm_security_lab/`](../labs/08_llm_security_lab/) — 프롬프트 인젝션(Jailbreak), RAG 간접 주입, 에이전트 도구 남용, 시스템 프롬프트 추출 실습 랩 (`python3 vhack.py lab start 08`, 웹 콘솔: `http://localhost:8088`)
> - **워게임 트랙**: 워게임 터미널(`wargame/`)의 `ai` 트랙 (35개 문제)에서 프롬프트 탈취, 적대적 예제, 모델 역공학, 탈옥 페이로드 문제를 직접 실습할 수 있습니다.
