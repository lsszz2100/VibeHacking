...
## 5.5 군사위성 FIPS 적용 사례: ANASIS-II (연구 결과)

> **출처**: 명지대 박사과정 — satellite_pqc 구현 연구 (2026-05), NIST FIPS 203/204/205 기준

ANASIS-II 정지궤도 군사위성(NORAD 44714)에 FIPS 203/204/205를 적용한 실측 데이터:

**알고리즘 선택 및 보안 요구사항 충족**

| FIPS 표준 | 선택 알고리즘 | 용도 | 핵심 실측값 | SR 충족 |
|-----------|-------------|------|-----------|--------|
| FIPS 203 | ML-KEM-768 | TT&C 세션 키 교환 | Decap 0.022ms | SR-01 (Cat.3) ✓ |
| FIPS 204 | ML-DSA-65 | 위성 명령 서명·검증 | Verify 0.052ms | SR-02 (5ms) ✓ |
| FIPS 205 | SLH-DSA-SHA2-192s | OTA 펌웨어 서명 | OBC 검증 1.32ms | SR-03 (Stateless) ✓ |

**FIPS 203 (ML-KEM) 파라미터 선택 이유**
- ML-KEM-768 → NIST Level 3 (AES-192 동급), 공개키 1,184B, S-band 오버헤드 9.09ms
- ML-KEM-1024도 검토했으나 오버헤드 12.77ms로 768 대비 40% 증가
- **TVLA 주의**: macOS liboqs 환경에서 ML-KEM-768 t-stat=10.91 (임계값 4.5) FAIL
  → OBC(LEON4FT) 하드웨어 환경에서 별도 재측정 필요

**FIPS 205 (SLH-DSA) 적용 패턴**
SLH-DSA는 서명이 776ms로 OBC 탑재 서명 불가 → 지상국 HSM 전용 서명, OBC는 1.32ms 검증만 수행.
이 분리 패턴이 위성 환경에서 Stateless 서명(SR-03: 재부팅 후 상태 손실 없음)을 달성하는 유일한 실용적 방법.
