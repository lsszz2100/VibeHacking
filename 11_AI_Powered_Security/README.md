...
- GPT-5.4-Cyber, Claude Opus 4.6 등 전문 보안 AI 모델 활용
...
Anthropic Mythos   → 17년 된 FreeBSD RCE 취약점 자율 발견/익스플로잇
                   → 모든 주요 OS/브라우저에서 수천 개 제로데이 식별
                   → 인간 최고수 버그헌터도 못 찾는 취약점 수만 개

OpenAI GPT-5.4-Cyber → CTF 성능 27% → 76% (GPT-5 → GPT-5.1-Codex-Max)
                     → 소스코드 없이 바이너리 리버싱
                     → YARA 룰 자동 생성
