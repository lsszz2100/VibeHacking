...
> 💡 **바이브해킹 실습 랩 (AI Shield) 및 워게임 연계**:
> - **인터랙티브 실습 랩 08**: [`labs/08_llm_security_lab/`](../labs/08_llm_security_lab/) — 시스템 프롬프트 탈취(Jailbreak), 마크다운 XSS, RAG 지식 베이스 간접 주입, 과도한 에이전트 권한 남용 통제 실습 랩 (`python3 vhack.py lab start 08`, 웹 콘솔: `http://localhost:8088`)
> - **워게임 트랙**: 워게임 터미널(`wargame/`)의 `ai` 트랙 (35개 문제)에서 탈옥 페이로드, 적대적 프롬프트 조작, 간접 주입 기법 문제를 직접 실습할 수 있습니다.
