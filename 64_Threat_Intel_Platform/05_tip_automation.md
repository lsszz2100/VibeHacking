...
**TIP(Threat Intelligence Platform) 자동화**는 위협 인텔리전스 수집, 처리, 배포 과정을 자동화하는 것입니다.

```
수동 프로세스:
  매일 아침 10개 위협 피드 수동 확인
  → 엑셀에 IOC 정리
  → 방화벽 담당자에게 이메일
  → 방화벽 수동 업데이트
  → 총 소요 시간: 2-3시간/일
...
자동화 프로세스:
  위협 피드 자동 수집 (cron/webhook)
  → 중복/만료 IOC 자동 필터링
  → MISP에 자동 등록
  → SIEM/방화벽에 API로 자동 배포
  → 총 소요 시간: 0분 (실시간)
```
...
SOAR (Security Orchestration, Automation and Response):
  보안 워크플로우 자동화에 집중
  사고 대응, 티켓팅, 다양한 도구 오케스트레이션
  
실제:
  TIP → IOC 제공 → SOAR → 사고 대응 자동화
  (서로 보완적)
```
...
External feeds → Collector → Normalization → Enrichment → Deduplication → Validation
                                                                          ↓
Distribute ← Expiry management ← IoC DB ← Confidence scoring ← Contextualization
↓
SIEM lookup / SOAR playbook / Firewall block / MISP sharing
```

## Integrated TIP Automation Pipeline

```python
#!/usr/bin/env python3
"""Integrated threat intelligence automation pipeline."""
...
class TIPPipeline:
    ...
    def collect(self) -> list[dict]:
        ...
        def fetch_feed(feed: dict) -> list[dict]:
            try:
                ...
                logger.info(f"[{feed['name']}] {len(iocs)} collected")
                return iocs
            except Exception as e:
                logger.warning(f"[{feed['name']}] collection failed: {e}")
                return []
    ...
    def enrich(self, iocs: list[dict]) -> list[dict]:
        """Enrich IoCs (VirusTotal etc.)."""
        if not self.config.vt_api_key:
            return iocs

        enriched: list[dict] = []
        # Free VT API: 4 requests per minute limit
        for i, ioc in enumerate(iocs[:50]):  # Enrich first 50 only
            if i > 0 and i % 4 == 0:
                time.sleep(60)  # Wait for per-minute limit
            ...
            try:
                with urllib.request.urlopen(req, timeout=10) as resp:
                    ...
                    ioc["vt_score"] = f"{malicious}/{total}"
                    ioc["vt_malicious"] = malicious > 3
                    # Raise confidence if confirmed malicious
                    if malicious > 10:
                        ioc["confidence"] = min(100, ioc["confidence"] + 20)
            except Exception:
                pass
    ...
    def validate(self, iocs: list[dict]) -> list[dict]:
        """Validate IoCs (minimum confidence, FP check)."""
        valid: list[dict] = []
        for ioc in iocs:
            if ioc["confidence"] < self.config.min_confidence:
                continue
            # Check FP report count from DB
            row = self.conn.execute(
                "SELECT fp_count FROM iocs WHERE value = ? AND ioc_type = ?",
                (ioc["value"], ioc["ioc_type"]),
            ).fetchone()
    ...
    def distribute(self) -> dict[str, int]:
        """Distribute active IoCs to each target."""
        results: dict[str, int] = {}
        rows = self.conn.execute(
            "SELECT * FROM iocs WHERE active = 1"
        ).fetchall()
    ...
    def run(self) -> PipelineResult:
        ...
        logger.info("1/5 Collecting...")
        collected = self.collect()
        logger.info(f"   {len(collected)} collected")

        logger.info("2/5 Deduplicating...")
        deduped = self.deduplicate(collected)
        logger.info(f"   {len(deduped)} (after deduplication)")

        logger.info("3/5 Enriching...")
        enriched = self.enrich(deduped)

        logger.info("4/5 Validating...")
        validated = self.validate(enriched)
        logger.info(f"   {len(validated)} valid")

        logger.info("5/5 Storing and distributing...")
        new_count = self.store(validated)
        distributed = self.distribute()
        expired = self.expire_old() if self.config.auto_expire else 0
        ...
        logger.info(f"[Pipeline {run_id}] Completed — {end - start:.1f}s")
        logger.info(f"   New: {new_count} | Expired: {expired} | Distributed: {result.distributed}")
        return result
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TIP automation pipeline")
    parser.add_argument("--db", type=Path, default=Path("tip.db"))
    parser.add_argument("-o", "--output", type=Path, default=Path("output"))
    ...
    parser.add_argument("--formats", nargs="*", default=["csv", "txt"])
    parser.add_argument("--daemon", action="store_true",
                        help="Run periodically (1-hour interval)")
    parser.add_argument("--interval", type=int, default=3600,
    ...
    if args.daemon:
        logger.info(f"Daemon mode: {args.interval}s interval")
        while True:
            pipeline = TIPPipeline(config)
            try:
                result = pipeline.run()
                logger.info(f"Result: {result.after_validation} valid IoCs")
            finally:
                pipeline.close()
            time.sleep(args.interval)
    else:
        ...
        try:
            ...
            print(f"  Deduplicated: {result.after_dedup}")
            print(f"  Validated:    {result.after_validation}")
...
```python
#!/usr/bin/env python3
"""SOAR playbook integration — Shuffle example."""
...
```
Daily tasks
☐ Confirm automated feed collection ran successfully
☐ Confirm new IoCs are reflected in SIEM
☐ Review false positive reports
☐ Clean up expired IoCs

Weekly tasks
☐ Evaluate feed quality (FP rate, coverage)
☐ Update tracking of new threat actors
☐ Review SIEM detection rule effectiveness
☐ Produce threat intelligence report

Monthly tasks
☐ Review adding/removing TIP feeds
☐ Update intelligence requirements (PIR)
☐ Evaluate ROI of collected intelligence
☐ Share intelligence with other organizations (ISAC)
```
