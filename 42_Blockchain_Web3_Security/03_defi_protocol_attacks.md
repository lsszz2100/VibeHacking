...
**DeFi(탈중앙화 금융) 프로토콜 공격**은 스마트 컨트랙트 취약점 또는 경제적 모델의 허점을 이용해 수백만~수천억 달러 규모의 자금을 탈취하는 공격입니다. 전통 금융과 달리 되돌릴 수 없습니다.
...
년도  프로토콜          피해액      공격 유형
2020  bZx              $8M        플래시 론 + 오라클 조작
2021  Compound         $150M      거버넌스 공격
2022  Ronin Bridge     $625M      키 관리 실패
2022  Nomad Bridge     $190M      로직 오류
2023  Euler Finance    $197M      플래시 론 + 청산 버그

공통점: 스마트 컨트랙트는 코드가 곧 법 → 버그 = 공격자 이익
```
...
플래시 론(Flash Loan):
  담보 없이 수천만 달러 순간 대출
  조건: 같은 트랜잭션 내에서 반환
  활용: 가격 조작 + 취약점 익스플로잇 조합

오라클 조작:
  DeFi는 외부 가격 정보(오라클) 의존
  오라클 가격 조작 → 인위적 가격 → 차익 탈취

재진입 공격:
  인출 함수 호출 → 잔액 차감 전 재호출
  → 실제 잔액보다 더 많이 출금
```

### 필요한 도구
- **Foundry**: 로컬 EVM 포크 + 공격 시뮬레이션
- **Hardhat**: JavaScript 기반 테스트 환경
- **Tenderly**: 트랜잭션 시뮬레이션·디버깅

### 기초 실습 예제
```python
# Web3.py로 플래시 론 트랜잭션 추적
from web3 import Web3

w3 = Web3(Web3.HTTPProvider("https://mainnet.infura.io/v3/YOUR_KEY"))

# bZx 해킹 트랜잭션 분석 (실제 해시)
tx_hash = "0xb5c8bd9430b6cc87a0e2fe110ece6bf527fa4f170a4bc8cd032f768fc5219838"
tx = w3.eth.get_transaction(tx_hash)

print(f"블록: {tx['blockNumber']}")
print(f"가스 사용: {tx['gas']:,}")
print(f"입력 데이터 크기: {len(tx['input'])} bytes")
# 입력 데이터 분석으로 어떤 함수를 호출했는지 파악
```
