...
**인시던트 대응(Incident Response, IR)**은 보안 사고 발생 시 피해를 최소화하고 신속하게 복구하는 체계적인 절차입니다.

**인시던트(Incident)란?**
```
보안 이벤트 vs 인시던트:
  이벤트: 보안과 관련된 모든 관찰 가능한 사건
          (예: 로그인 시도, 방화벽 차단)
  
  인시던트: 보안 정책을 위반하거나 위반 시도
            (예: 성공한 침해, 데이터 유출)
...
IR 없을 때:
  발견: 직원이 파일 암호화됨을 눈치챔 (수일 후)
  대응: 모두 패닉, 어디서부터 시작할지 모름
  결과: 랜섬웨어가 네트워크 전체로 확산
        복구 불가능한 데이터 손실
        3주 후에야 정상 운영
  
IR 있을 때:
  발견: SIEM이 이상 행동 탐지 (수분 내)
  대응: 플레이북 따라 즉시 격리
  결과: 감염 범위 최소화
        포렌식 증거 보존
        3일 후 정상 운영
...
### NIST SP 800-61r2 프레임워크

**미국 국립표준기술연구소(NIST)**의 컴퓨터 보안 사고 처리 가이드:
